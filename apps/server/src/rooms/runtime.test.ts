...
describe('concurrent arcade games wiring (space-invaders, bomberman & Puzzle Bubble)', () => {
  ...
  it('wires Block Blaster as a concurrent score game without turns or tick watchdog', () => {
    ...
    const view = room.engine().view(room.gameState as never, 'p1') as {
      you: { tray: Array<{ cellCount: number } | null> };
    };
    const piece = view.you.tray[0];
    if (!piece) throw new Error('expected first Block Blaster tray piece');
