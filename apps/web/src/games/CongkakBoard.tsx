...
import { FastForward, Sparkles, Trophy, Zap } from 'lucide-react';
...
/**
 * Computes the full step-by-step marble distribution sequence.
 */
function generateSowSteps(
  ...
  // Step 0: Pickup from chosen house
  steps.push({
    pits: [...currentPits],
...
export function CongkakBoard({
  ...
  // Persistent simulation board ref that never resets mid-queue
  ...
  // Move queue and deduplication tracking
  ...
  // Keep latest view in a ref for safe access inside timer callbacks
  const latestViewRef = React.useRef(view);
  latestViewRef.current = view;

  // Process the animation queue step by step
  const pumpQueue = React.useCallback(() => {
    ...
    if (moveQueueRef.current.length === 0) {
      ...
      setActiveStep(null);
      // Sync display with authoritative server state once all queued moves are done
      setDisplayBoard({
    ...
    // Update simulated state so the NEXT queued move calculates from the correct pre-state
    ...
    activeTimerRef.current = setInterval(() => {
      ...
      if (stepIdx >= steps.length) {
        ...
        isRunningRef.current = false;
        // Pause briefly at the end of the move before starting the next move in queue
        setTimeout(() => pumpQueue(), 250);
        return;
      }
      applyStep(stepIdx);
    }, 500); // Exactly 0.5s per marble drop
  }, [nPits]);
  ...
  // Play Congkak background music
  React.useEffect(() => {
    bgm.play('congkak');
  }, []);

  // Enqueue moves coming from server updates (e.g. Bot or Opponent moves)
  React.useEffect(() => {
    if (!view.lastMove) {
  ...
  // Pits indices based on orientation
  ...
  const handleSow = (pitIdx: number) => {
    ...
    // Enqueue human move locally so it animates immediately without waiting for network roundtrip
